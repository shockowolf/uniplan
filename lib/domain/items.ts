    ...
    async (databaseTransaction) => {
      ...
      return databaseTransaction.item.create({
        data: {
          ...
          standardPrice: nonnegativeMoney(
...
export async function updateItem(
  ...
  return withCompanyMutationTransaction(
    ...
    async (databaseTransaction) => {
      ...
      const trackInventory =
        input.trackInventory ?? existingItem.trackInventory;
...
export async function createItemCategory(
  ...
  return withCompanyMutationTransaction(
    ...
    async (databaseTransaction) => {
      if (input.parentId) {
        const parentItemCategory =
          await databaseTransaction.itemCategory.findFirst({
...
export async function deactivateItemCategory(
  ...
  return withCompanyMutationTransaction(
    ...
    databaseClient,
    async (databaseTransaction) => {
      const itemCategory = await databaseTransaction.itemCategory.findFirst({
      ...
      if (input.parentId !== undefined && input.parentId !== null) {
        ...
        while (ancestorCategoryId) {
          const ancestorCategory: {
            id: string;
            parentId: string | null;
          } | null = await databaseTransaction.itemCategory.findFirst({
